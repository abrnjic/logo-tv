    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('#EXTINF:')) {
          totalChannels++;
          // Parse channel name, usually after the last comma
          const commaIndex = line.lastIndexOf(',');
          if (commaIndex !== -1) {
            const rawName = line.substring(commaIndex + 1).trim();
            const searchJoined = rawName.toLowerCase().replace(/[-_.\s]/g, '');
            const searchWords = rawName.toLowerCase().replace(/[-_.]/g, ' ').split(/\s+/).filter(w => w.length > 0);

            // Find match
            const match = channelsData.find(ch => {
              const joinedName = ch.name.toLowerCase().replace(/[-_.\s]/g, '');
              if (joinedName === searchJoined) return true;
              
              const cleanName = ch.name.toLowerCase().replace(/[-_.]/g, ' ');
              if (searchWords.length > 0 && searchWords.every(word => cleanName.includes(word))) return true;
